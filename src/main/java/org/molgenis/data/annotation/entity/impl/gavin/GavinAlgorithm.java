...
import static org.molgenis.data.annotation.entity.impl.gavin.Category.I1;
import static org.molgenis.data.annotation.entity.impl.gavin.Category.I2;
import static org.molgenis.data.annotation.entity.impl.gavin.Category.I3;
...
public class GavinAlgorithm
{
			...
			GavinThresholds gavinThresholds)
	{
			...
			switch (gavinThresholds.getCategory())
			{
				...
				case C5:
					if (gavinThresholds.isAboveSpec95thPerCADDThreshold(caddScaled))
					{
						return new Judgment(Pathogenic, calibrated, gene,
								"Variant CADD score of " + caddScaled + " is greater than "
										+ gavinThresholds.getSpec95thPerCADDThreshold() + " for this gene.",null, null);
					}
					if (gavinThresholds.isBelowSens95PerCADDThreshold(caddScaled))
					{
						return new Judgment(Benign, calibrated, gene,
								"Variant CADD score of " + caddScaled + " is less than "
										+ gavinThresholds.getSens95thPerCADDThreshold() + " for this gene.",null, null);
					}
					//this rule does not classify apparently, just continue onto the next rules
					break;
			}
		...
		{
			return new Judgment(Benign, calibrated, gene,
					"Variant MAF of " + exacMAF + " is greater than " + gavinThresholds.getPathoMAFThreshold() + ".",null, null);
		}
